...
public class BaristaModel {
    ...
    public DefaultTableModel getDataMinuman() {
        ...
        try {
            ...
            String sql = "SELECT d.id_pesanan, m.nama_menu AS menu, d.qty, s.status AS status_pesanan, r.status AS status_minuman, d.deskripsi " +
                         "FROM detail_pesanan d " +
                         "JOIN status s ON d.id_status = s.id_status " +
                         "JOIN status_barista r ON d.id_status_minuman = r.id_status_minuman " +
                         "JOIN pesanan p ON d.id_pesanan = p.id_pesanan " +
                         "JOIN menu m ON d.id_menu = m.id_menu " +
                         "WHERE m.kategori = 'beverage' AND DATE(p.tanggal) = CURDATE()" +
